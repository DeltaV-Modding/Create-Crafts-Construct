...
public class PaintedFluidValveRenderer extends KineticBlockEntityRenderer<FluidValveBlockEntity> {
    private static final Map<String, PartialModel> POINTERS = new HashMap<>();

    public PaintedFluidValveRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }
    ...
    static PartialModel getPointer(BlockState state) {
        ...
        PartialModel pointer = net.buildercraft.registry.ModPartialModels.VALVE_POINTERS.get(path);
        if (pointer == null) {
