...
/**
 * Renders the fluid stream inside painted fluid pipes when in glass mode.
 * Logic is copied from GlassPipeVisual since it's typed to StraightPipeBlockEntity.
 */
public class AndesiteGlassPipeVisual extends AbstractBlockEntityVisual<AndesiteFluidPipeBlockEntity>
    ...
    public AndesiteGlassPipeVisual(VisualizationContext ctx, AndesiteFluidPipeBlockEntity blockEntity, float partialTick) {
    ...
    public void beginFrame(Context ctx) {
        ...
        // Only render when in glass mode
        if (!blockEntity.getBlockState().hasProperty(AndesiteFluidPipeBlock.GLASS) ||
                !blockEntity.getBlockState().getValue(AndesiteFluidPipeBlock.GLASS)) {
            stream.discardExtra();
        ...
        for (Direction side : Iterate.directions) {
            ...
            int lightLevel = (this.light & 0xF00000) | luminosity << 4;
            ...
            fluidInstance.light(lightLevel).colorArgb(color);
