 ...
 */
public class BrassGlassPipeRenderer extends SafeBlockEntityRenderer<BrassFluidPipeBlockEntity> {

    public BrassGlassPipeRenderer(BlockEntityRendererProvider.Context context) {
    }

    @Override
    protected void renderSafe(BrassFluidPipeBlockEntity be, float partialTicks, PoseStack ms,
                              MultiBufferSource buffer, int light, int overlay) {
        BlockState state = be.getBlockState();
        if (!state.hasProperty(BrassFluidPipeBlock.GLASS) || !state.getValue(BrassFluidPipeBlock.GLASS))
            return;
